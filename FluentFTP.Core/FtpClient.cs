...
namespace FluentFTP {
	...
	public class FtpClient : IDisposable {
        ...
        private SslProtocols m_SslProtocols = SslProtocols.Tls | SslProtocols.Ssl3;
        /// <summary>
        ...
        public bool ExistsAsync(string path)
        {
            if (!IsConnected)
            {
            ...
            {
                throw new ArgumentOutOfRangeException("path", "path supplied was incorrect");
            }
            if (path.EndsWith("/"))
            {
        ...
        public static FtpClient Connect(Uri uri, bool checkcertificate) {
            ...
            if (uri.UserInfo != null && uri.UserInfo.Length > 0) {
                if (uri.UserInfo.Contains(":")) {
                    ...
                    cl.Credentials = new NetworkCredential(WebUtility.UrlDecode(parts[0]), WebUtility.UrlDecode(parts[1]));
                }
                else
                    cl.Credentials = new NetworkCredential(WebUtility.UrlDecode(uri.UserInfo), "");
            }
            else {
